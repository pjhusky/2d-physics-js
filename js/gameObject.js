...
class GameObject {
    ...
    getPos() { 
        return this.rigid_body.pos_vec2; 
    }
    ...
    getAngle() { 
        return this.rigid_body.angle_rad; 
    }
    ...
    setDebugInfoVisibility( flag ) { 
        this.render_primitive.setDebugInfoVisibility( flag ); 
    }

    resetFillColor ( /*fill_color*/ ) {
        // const fill_color = [ 0.2, 0.2, 0.2, 0.9 ];
        //this.render_primitive.setFillColor( fill_color );
        
        this.render_primitive.resetFillColor();
    }
}

// export
class GameObject_Breakable extends GameObject {
    constructor( rigid_body, render_primitive, break_sound_alias, collide_callback ) {
        super( rigid_body, render_primitive );
        this.shattered = false;
        this.break_sound_alias = break_sound_alias;
    ...
    onCollide( collision_info ) {
        ...
        // play shatter sound
        PIXI.sound.play( this.break_sound_alias, {loop: false});
        
        if ( this.collide_callback != undefined) {
        ...
        //const mass = 0.0;
        const entire_mass = SimulationParameters.rigidBodyDefaultMass() * 10.0; // NOTE: usually we set the breakable to 0 mass == fixed!
        //const mass = 0.5;
        ...
        let points_vec2 = [];
        this.rigid_body.relative_path_points_ccw.forEach( (point_array2) => {
            points_vec2.push( Vec2.fromArray( point_array2 ) );
        });
        const area_before_break = RigidBody_Polygon.calculateArea( points_vec2 );
        console.log( `area_before_break = ${area_before_break}` );

        
        let cell_idx = 0;
        console.log( `there are ${output_voronoi_cells_array2.length} voronoi cells:` )
        output_voronoi_cells_array2.forEach( (voronoi_cell) => {
            //console.log( `------ voronoi cell '${cell_idx}' with ${voronoi_cell.length} vertices` ); 
            cell_idx++;
            ...
            voronoi_cell.forEach( (vertex_array2) => {
                //console.log( `( ${vertex[0]} | ${vertex[1]} )` );
                com_vec2.add( Vec2.fromArray(vertex_array2) );
                
                points_vec2.push( Vec2.fromArray( vertex_array2 ) );
            } );
            const num_verts = voronoi_cell.length;
            const scale = 1.0 / num_verts;
            com_vec2.mulScalar( scale );

            const cell_area = RigidBody_Polygon.calculateArea( points_vec2 );
            console.log( `cell ${cell_idx} has area ${cell_area}` );
            
            const mass = ( cell_area / area_before_break ) * entire_mass;

            
            //const shrink_factor = 0.925;
            const shrink_factor = 0.9125;
    ...
    resetFillColor () {
        const fill_color = [ 0.75, 0.7, 0.0, 0.9 ];
        this.render_primitive.setFillColor( fill_color );
    }
