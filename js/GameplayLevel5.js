...
class GameplayLevel5 {
	...
	initPictureParticles() {
		...
		for (let i = 0; i < this.maxNumFluidParticles; i++) {
			...
			// 	if ( useStickyGooLook ) {
			// 		fluidParticle.tint = parseInt(Math.floor(GameplayLevel5.random() * 16777215).toString(16), 16); // that STICKY gooey look :-)
			// 	} else {
			...
			// } else {
				...
				if ( addr < fluidParticleTints.length ) {
					...
					fluidParticle.tint = parseInt(Math.floor(GameplayLevel5.random() * 16777215).toString(16), 16);
			...
			fluidParticle.position.x = this.sim_grid_dim.w * ( 0.33 + GameplayLevel5.random()*0.33 );
			fluidParticle.position.y = this.sim_grid_dim.h * ( 0.33 + GameplayLevel5.random()*0.33 );

			fluidParticle.speed = 1 + GameplayLevel5.random() * 1.2;
  	...
  	onStart(container) {
		...
		// Level 5 info text
		const levelInfo = new PIXI.Text('Level 4', levelInfoStyle);
		levelInfo.x = 20;
		levelInfo.y = 20;
		...
		if ( this.recording ) {
			...
			this.container.filters = [passThroughFilter];
	...
	updatePictureParticles(dt_in_sec) {
		...
		for (let i = 0; i < emissionRate; i++) {
			...
			if (this.activeFluidParticles < this.maxNumFluidParticles) {
				...
				// 	// randDir = GameplayLevel5.random() * 0.5 - 0.25;
				...
				// 	let signX = ( activeFluidParticles % 3 ) - 1;
				// 	let randDir = GameplayLevel5.random() * 0.5 - 0.25;
	...
	startWebWorker() {
		...
		// run physics web worker
		this.physicsWorker = new Worker('../js/WorkerLevel5.js');
